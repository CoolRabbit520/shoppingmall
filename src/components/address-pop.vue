                            ...
                            <div class="address-form">
                                <div class="module-form-row">
                                    <div class="form-item-v3">
                                        <input v-model="receiveInfo.name" placeholder="收货人姓名" type="text" class="js-verify">
                                        <div class="verify-error"></div>
                                    </div>
                                </div>
                                <div class="module-form-row">
                                    <div class="form-item-v3" :class="{'form-invalid-item':phoneError}"  >
                                        <input v-model="receiveInfo.phone" placeholder="手机号" @focus="setPhoneError" @blur="inspectPhone" type="text" class="js-verify">
                                        <div class="verify-error" v-show="phoneError">手机号格式错误</div>
                                    </div>
                                </div>
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 area-code-w fn-left form-valid-item">
                                        <input type="text" v-model="receiveInfo.countyId" placeholder="区号（可选）" class="js-verify js-address-area-code">
                                        <div class="verify-error"></div>
                                    </div>
                                    <div class="form-item-v3 telephone-w fn-right form-valid-item">
                                        <input type="text" v-model="receiveInfo.landLine" placeholder="固定电话（可选）" class="js-verify js-address-telephone">
                                        <div class="verify-error" ></div>
                                    </div>
                                </div>
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 select-item province-wrapper">
                                        <select name="province_code" v-model="receiveInfo.provinceId" class="province select-provijs-form-province js-verify">
                                ...
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                        <select v-model="receiveInfo.cityId" class="city select-city js-form-city js-verify">
                                            <option value="0">请选择城市</option>
                                            <option :value="item.area_id"  v-for="(item ,index) in cityList">{{item.area_name}}</option>
                                        </select>
                                    </div>
                                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-">
                                        <select v-model="receiveInfo.countyId" class="city select-city js-form-city js-verify">
                                ...
                                <div class="module-form-row">
                                    <div class="form-item-v3">
                                        <input placeholder="详细地址，如街道名称，楼层，门牌号码等" v-model="receiveInfo.add" type="text" class="js-verify">
                                        <div class="verify-error"></div>
                                    </div>
                                </div>
                                <div class="module-form-row fn-clear" >
                                    <input type="checkbox" class="hide">
                                    <span class="blue-checkbox " @click="checkDefault" :class="{'blue-checkbox-on':receiveInfo.default}"></span>设为默认
                                </div>
                                <div class="dialog-blue-btn big-main-btn  js-verify-address" :class="{'disabled-btn':!isRight}" @click="submitHandle">
                                    <a>保存</a>
                                </div>
                            </div>
                        </div>
...
export default{
    ...
    watch:{
        "receiveInfo.countyId"(){
            this.countyList.forEach(item=>{
                if(item.area_id==this.receiveInfo.countyId){
                    this.receiveInfo.county=item.area_name
                    return
                }
            })
        },
    ...
    computed:{
        cityList(){
            ...
            this.addList.forEach(item=>{
                if(item.area_id==this.receiveInfo.provinceId){
                    cityList=item.city_list
                    this.receiveInfo.province=item.area_name
                    return
                }
            })
            if(this.receiveInfo.provinceId==="0"){
        ...
        countyList(){
            ...
            this.cityList.forEach(item=>{
                if(item.area_id==this.receiveInfo.cityId){
                    countyList=item.county_list
                    this.receiveInfo.city=item.area_name
                    return
                }
            })
            if(this.receiveInfo.cityId==="0"){
                this.receiveInfo.countyId="0"
            }
            return countyList
        }
    ...
    methods:{
        ...
        checkDefault(){
            ...
            console.log( this.receiveInfo.default)
        },
        inspectReceive(){
            if(this.receiveInfo.name && !this.phoneError && this.receiveInfo.province && this.receiveInfo.city && this.receiveInfo.county && this.receiveInfo.add){
                this.isRight=true
            }else{
                this.isRight=false
            }
        ...
        submitHandle(){
            if(this.isRight){
                this.$store.commit('submitHandle',this.receiveInfo)
                this.$emit('close')
            }
        }
