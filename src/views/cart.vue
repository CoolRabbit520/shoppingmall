											...
											<div class="cart-items">
												...
												<div class="item-cols-num">
													<div class="select js-select-quantity">
														<span class="down" @click="subCartPanelDataHandle(item.sku_id)" :class="{'down-disabled':item.count==1}">-</span>
														<span class="num">{{item.count}}</span>
														<span class="up" @click="plusCartPanelDataHandle(item.sku_id)" :class="{'up-disabled':item.count==item.limit_num}">+</span>
...
<script>
	export default{
		...
		methods:{
			...
			plusCartPanelDataHandle(id){
				return this.$store.commit('plusCarPanelData',id)
			},
			subCartPanelDataHandle(id){
				return this.$store.commit('subCarPanelData',id)
			},
			checkGoodsHandle(id){
				return this.$store.commit('checkGoods',id)
			},
			allCheckGoodsHandle(allChecked){
				return this.$store.commit('allCheckedGoods',allChecked)
			},
			delCheckedGoodsHandle(){
				return this.$store.commit('delCheckedGoods')
			}
