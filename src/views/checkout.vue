<template>
	<div id="main">
					...
					<div class="box-inner js-checkout-address-panel ">
						<div class="address-common-table js-multiple-address-panel">
							<ul class="address-item-list clear js-address-item-list">
								<li class="js-choose-address " @click="chooseReceive(index)" :class="{'selected-address-item':receiveIndex==index}" v-for="(item,index) in receiveInfo">
					...
					<div class="invoice-detail"> 发票抬头： 
						<div class="radio-box"> 
							<label> 
								...
								<span class="blue-radio" @click="checkedInvoice(true)" :class="{' blue-radio-on':invioce.personal}"><a></a></span>  个人
							</label> 
							<label> 
								<input type="radio" class="hide"> 
								<span class="blue-radio" @click="checkedInvoice(false)" :class="{' blue-radio-on':!invioce.personal}"><a></a></span>  单位
							</label> 
						</div> 
						<div class="module-form-row form-item fn-hide js-invoice-title" v-if="!invioce.personal"> 
							<div class="module-form-item-wrapper no-icon small-item"> 
								<i v-show="!invioce.name">请填写公司发票抬头</i> 
								<input  v-model="invioce.name" type="text" class="js-verify"> 
								<div class="verify-error" v-show="!invioce.name">必填</div>
							</div> 
						</div> 
					</div> 
						...
						<div class="cart-group js-cart-group">
							<div class="cart-items" v-for="(item,index) in checkedGoods">
								...
								<div class="name hide-row">
									<div class="name-cell"> 
										<a href="javascript:;" title="item.title+'（'+item.color+'）'" target="_blank">{{item.title}}{{item.spec_json.show_name}}</a>
									</div>
								</div>
								<div class="subtotal">
		...
		<addressPop v-show="popShow" @close="closePop"></addressPop>
	</div>
</template>
...
export default{
	...
	methods:{
		...
		checkedInvoice(boolean){
			this.invioce.personal=boolean
		},
		submitOrderHandle(){
			if(!this.invioce.personal &&  !this.invioce.name) return
			let receiveInfo=this.receiveInfo[this.receiveIndex]
			if(this.invioce.personal){
				this.invioce.name="个人"
			}
			let iDate= new Date()
