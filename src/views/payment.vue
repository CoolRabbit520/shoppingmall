<template>
	<div id="main">
        ...
        <p class="info-detail">姓名：{{orderInfo.receiveInfo.name}}</p>
		        ...
		        <p class="info-deta">详细地址：{{orderInfo.receiveInfo.province}}{{orderInfo.receiveInfo.city}}{{orderInfo.receiveInfo.county}}{{orderInfo.receiveInfo.add}}</p>
		    </div>
		    <div class="confirm-detail">
		        <div class="info-title">发票信息</div>
		        <p class="info-detail">发票类型：电子发票</p>
		        <p class="info-detail">发票抬头：{{orderInfo.invioceName}}</p>
...
export default{
	computed:{
		orderInfo(){
			let orderInfo=this.$store.state.orderData.filter(item=>{
